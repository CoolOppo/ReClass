...
{
	m_brBackGnd = CreateSolidBrush(0x80ffff);
	return CEdit::Create(dwStyle, rect, pParentWnd, nID);
}
HBRUSH myCToolTip::CtlColor(CDC* pDC, UINT nCtlColor)
{
	pDC->SetBkColor(0x80ffff);
	return m_brBackGnd;
}
	...
	for ( UINT i=0; i < HotSpots.size( );i++ )
	{
		if ( HotSpots[i].Rect.PtInRect( point ) )
		{
			if (HotSpots[i].Type == HS_EDIT )
			{
				...
				// I have no idea what is going on here but I assume that
				// this control isn't created with ES_AUTOHSCROLL - ICY

				m_Edit.SetWindowPos( NULL,	HotSpots[i].Rect.left, 
											HotSpots[i].Rect.top,
											HotSpots[i].Rect.Width( ), // + 80,
											HotSpots[i].Rect.Height( ),
											SWP_NOZORDER );
			...
			if ( (HotSpots[i].Type == HS_CHANGE_A) || (HotSpots[i].Type == HS_CHANGE_X) )
			{
				...
				{
					if ( (HotSpots[i].Type == HS_CHANGE_X) && (pNode->pParent == theApp.Classes[m]) ) continue;
...
void CChildView::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	for (UINT i=0; i < HotSpots.size();i++)
	{
		if (HotSpots[i].Rect.PtInRect(point))
		{
			...
			if (HotSpots[i].Type == HS_CLICK)  pHitObject->Update(HotSpots[i]);
	...
	if (m_pClass->RequestPosition != -1)
	{
		if ((m_pClass->RequestPosition >= 0) && ( ( unsigned int ) m_pClass->RequestPosition < theApp.Classes.size()) )
		{
			...
			for (UINT i=0; i<theApp.Classes.size();i++)
			{
				...
				if (m_pClass == theApp.Classes[i]) idx = i;
			}
...
void CChildView::OnMouseHover(UINT nFlags, CPoint point)
{
	...
	{
		DWORD size = 0;
		for (UINT i=0; i<Selected.size();i++) size += Selected[i].object->GetMemorySize();
...
// TODO: make /8
// Or better yet make it == alignment
...
{
	vector<CNodeBase*> newSelected;

	//printf( "Replace Node Type %d\n", Type );

	for (UINT i=0; i < Selected.size();i++)
	{
		if (!theApp.IsNodeValid(Selected[i].object)) continue;
		if (Selected[i].object->pParent->GetType() == nt_vtable) Type = nt_function;
...
void CChildView::StandardTypeUpdate( CCmdUI *pCmdUI )
{
	if ( Selected.size( ) > 0 )
	{
		if ( Selected[0].object->GetType( ) == nt_class ) return pCmdUI->Enable( FALSE );
...
void CChildView::OnButtonRandom()
{
	...
	for (UINT i=0; i < Selected.size();i++)
	{
		...
		DWORD_PTR a = Selected[i].Address;
		mem.SetSize(s);
		for (UINT r=0; r < s; r++) mem.pMemory[r] = rand();
		WriteMemory(a,mem.pMemory,s);
	}
