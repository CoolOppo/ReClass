...
vector<MemMapInfo> MemMap;
vector<MemMapInfo> MemMapCode;
vector<MemMapInfo> MemMapData;
vector<MemMapInfo> MemMapModule;

vector<AddressName> Exports;
vector<AddressName> CustomNames;
...
CString GetAddressName(DWORD_PTR Address,bool bHEX)
{
	...
	if (bHEX)txt.Format("%I64X",Address);
	return txt;
}
