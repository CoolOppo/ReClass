...
// The one and only CReClass2011App object
CReClass2011App theApp;

// CReClass2011App
BEGIN_MESSAGE_MAP(CReClass2011App, CWinAppEx)
	ON_COMMAND(ID_APP_ABOUT, &CReClass2011App::OnAppAbout)
	ON_COMMAND(ID_FILE_NEW, &CReClass2011App::OnFileNew)
	ON_COMMAND(ID_FILE_IMPORT, &CReClass2011App::OnFileImport)
	ON_COMMAND(ID_BUTTON_NEWCLASS, &CReClass2011App::OnButtonNewclass)
	ON_COMMAND(ID_BUTTON_NOTES, &CReClass2011App::OnButtonNotes)
	ON_COMMAND(ID_BUTTON_PARSER, &CReClass2011App::OnButtonParser)
	ON_COMMAND(ID_BUTTON_HEADER, &CReClass2011App::OnButtonHeader)
	ON_COMMAND(ID_BUTTON_FOOTER, &CReClass2011App::OnButtonFooter)
	ON_COMMAND(ID_FILE_SAVE, &CReClass2011App::OnFileSave)
	ON_COMMAND(ID_FILE_SAVE_AS, &CReClass2011App::OnFileSaveAs)
	ON_COMMAND(ID_FILE_OPEN, &CReClass2011App::OnFileOpen)
	ON_COMMAND(ID_BUTTON_RESET, &CReClass2011App::OnButtonReset)
	ON_COMMAND(ID_BUTTON_PAUSE, &CReClass2011App::OnButtonPause)
	ON_COMMAND(ID_BUTTON_RESUME, &CReClass2011App::OnButtonResume)
	ON_COMMAND(ID_BUTTON_KILL, &CReClass2011App::OnButtonKill)
	ON_UPDATE_COMMAND_UI(ID_BUTTON_PAUSE, &CReClass2011App::OnUpdateButtonPause)
	ON_UPDATE_COMMAND_UI(ID_BUTTON_RESUME, &CReClass2011App::OnUpdateButtonResume)
	ON_UPDATE_COMMAND_UI(ID_BUTTON_KILL, &CReClass2011App::OnUpdateButtonKill)
	ON_COMMAND(ID_BUTTON_GENERATE, &CReClass2011App::OnButtonGenerate)
	ON_COMMAND(ID_BUTTON_CLEAN, &CReClass2011App::OnButtonClean)
	ON_UPDATE_COMMAND_UI(ID_BUTTON_CLEAN, &CReClass2011App::OnUpdateButtonClean)
	ON_UPDATE_COMMAND_UI(ID_FILE_SAVE, &CReClass2011App::OnUpdateFileSave)
END_MESSAGE_MAP()

CReClass2011App::CReClass2011App()
{
	m_bHiColorIcons = TRUE;
	m_dwRestartManagerSupportFlags = AFX_RESTART_MANAGER_SUPPORT_RESTART;
	SetAppID(_T("ReClass.2014.1b"));
...
extern int Scintilla_ReleaseResources(void);
...
void CReClass2011App::OnButtonReset()
{
...
void CReClass2011App::OnButtonPause()
{
	PauseResumeThreadList(false);
}
void CReClass2011App::OnUpdateButtonPause(CCmdUI *pCmdUI)
{
	pCmdUI->Enable((hProcess != NULL));
}

void CReClass2011App::OnButtonResume()
{
	PauseResumeThreadList(true);
}
void CReClass2011App::OnUpdateButtonResume(CCmdUI *pCmdUI)
{
	pCmdUI->Enable((hProcess != NULL));
}


void CReClass2011App::OnButtonKill()
{
	TerminateProcess(hProcess, 0);
	hProcess = NULL;
}
void CReClass2011App::OnUpdateButtonKill(CCmdUI *pCmdUI)
{
	pCmdUI->Enable((hProcess != NULL));
}

void CReClass2011App::CalcOffsets(CNodeClass* pClass)
{
	DWORD offset = 0;
...
void CReClass2011App::OnFileNew()
{
...
void CReClass2011App::OnAppAbout()
{
	CAboutDlg aboutDlg;
	aboutDlg.DoModal();
}

void CReClass2011App::PreLoadState()
{
	BOOL bNameValid;
	CString strName;
...
void CReClass2011App::LoadCustomState()
{}

void CReClass2011App::SaveCustomState()
{}
...
void CReClass2011App::OnFileImport()
{
	return;
	...
	vector<ImportNode> Import;
	vector<ImportLink> Links;
...
void CReClass2011App::ClearSelection()
{
	for(UINT i = 0; i < Classes.size(); i++) {
...
void CReClass2011App::ClearHidden()
{
	for(UINT i = 0; i < Classes.size(); i++) {
...
bool CReClass2011App::IsNodeValid(CNodeBase* pCheckNode)
{
	for(UINT i = 0; i < Classes.size(); i++) {
		for(UINT n = 0; n < Classes[i]->Nodes.size(); n++) {
			CNodeBase* pNode = Classes[i]->Nodes[n];
			...
			if(t == nt_vtable) {
				...
				for(UINT f = 0; f < pVTable->Nodes.size(); f++) {
					if(pVTable->Nodes[f] == pCheckNode) return true;

				}
			}
			if(t == nt_array) {
				CNodeArray* pArray = (CNodeArray*)pNode;
				if(pArray->pNode == pCheckNode) return true;
			}
			if(t == nt_pointer) {
				CNodePtr* pPtr = (CNodePtr*)pNode;
				if(pPtr->pNode == pCheckNode) return true;
			}
		}
	}
	return false;
...
void CReClass2011App::OnButtonNewclass()
{
	CMainFrame* pFrame = STATIC_DOWNCAST(CMainFrame, m_pMainWnd);
	CChildFrame* pChild = (CChildFrame*)pFrame->CreateNewChild(RUNTIME_CLASS(CChildFrame), IDR_ReClass2011TYPE, m_hMDIMenu, m_hMDIAccel);
...
void CReClass2011App::OnButtonNotes()
{
	CDialogEdit dlg;
	dlg.Title = "Notes";
	dlg.Text = Notes;
...
void CReClass2011App::OnButtonParser()
{
...
void CReClass2011App::OnButtonHeader()
{
	CDialogEdit dlg;
	dlg.Title = "Header";
	dlg.Text = Header;
...
void CReClass2011App::OnButtonFooter()
{
	CDialogEdit dlg;
	dlg.Title = "Footer";
	dlg.Text = Footer;
...
CNodeBase* CReClass2011App::CreateNewNode(NodeType Type)
{
	if(Type == nt_class) {
		// printf( "[+] Creating new class BASE: %p\n");
		return new CNodeClass;
	}


	if(Type == nt_hex64) return new CNodeHex64; // added by ICY
	...
	if(Type == nt_uint16) {
		//printf( "create new word\n" );
		return new CNodeWORD;
	}
	...
	if(Type == nt_pchar) return new CNodePChar; // added by ICY
...
void CReClass2011App::SaveXML(char* FileName)
{
	TiXmlDocument doc;
...
void CReClass2011App::OnFileSave()
{
	SaveXML(CurrentFilePath.GetBuffer());
}
void CReClass2011App::OnFileSaveAs()
{
...
void CReClass2011App::OnFileOpen()
{
	char szFilters[] = "ReClass (*.reclass)|*.reclass|All Files (*.*)|*.*||";
	...
	if(strcmp(v, "ReClass") != 0) return;//Not a Reclass file...
	...
	if(pElem)Header = pElem->Attribute("Text");

	pElem = hRoot.FirstChild("Footer").Element();
	if(pElem)Footer = pElem->Attribute("Text");

	pElem = hRoot.FirstChild("Notes").Element();
	if(pElem)Notes = pElem->Attribute("Text");
	...
	while(pElem) {
		...
		if(pClass->strOffset == "") pClass->strOffset = pElem->Attribute("Offset");
...
void CReClass2011App::OnButtonGenerate()
{
	CDialogEdit dlg;
	dlg.Title = "Headers";
	...
	for(UINT c = 0; c < Classes.size(); c++) {
		...
		for(UINT n = 0; n < pClass->Nodes.size(); n++) {
			...
			if((Type == nt_hex64) || (Type == nt_hex32) || (Type == nt_hex16) || (Type == nt_hex8)) {
				if(fill == 0) fillStart = pNode->offset;
			...
			if(Type == nt_instance) {
				...
				if(pCls->offset == 0) {
					t.Format(" : public %s", pCls->pNode->Name);
					ClassName += t;
				} else {
		...
		for(UINT i = 0; i < vfun.size(); i++) h += vfun[i];
		if(vfun.size()>0) h += "\r\n";

		for(UINT i = 0; i < var.size(); i++)	h += var[i];
		if(var.size()>0) h += "\r\n";
...
void CReClass2011App::DeleteClass(CNodeClass* pClass)
{
...
CNodeBase* CReClass2011App::isNodeRef(CNodeBase* pTestNode)
{
	for(UINT c = 0; c < Classes.size(); c++) {
...
void CReClass2011App::OnButtonClean()
{
	CMDIFrameWnd* pFrame = (CMDIFrameWnd*)AfxGetApp()->m_pMainWnd;
	CMDIChildWnd* wnd = pFrame->MDIGetActive();
	while(wnd) {
	...
	vector<CNodeClass*> toCheck;

	for(UINT i = 0; i < Classes.size(); i++) {
		if(isNodeRef(Classes[i]) == NULL) toCheck.push_back(Classes[i]);
	}
	...
	for(UINT i = 0; i < toCheck.size(); i++) {
		...
		for(UINT n = 0; n < pClass->Nodes.size(); n++) {
			...
			if(Type == nt_hex32) continue;
			if(Type == nt_hex16) continue;
			if(Type == nt_hex8) continue;
...
void CReClass2011App::OnUpdateButtonClean(CCmdUI *pCmdUI)
{
	pCmdUI->Enable((theApp.Classes.size() > 0));
}


void CReClass2011App::OnUpdateFileSave(CCmdUI *pCmdUI)
{
