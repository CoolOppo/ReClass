...
public:
	CNodeBase()
	{
		...
		Name.Format( "N%0.8X", GetTickCount( ) + NodeCreateIndex );
		NodeCreateIndex++;
		pParent = NULL;
	}
	~CNodeBase()
	...
	{
		if ( (y > View.client->bottom) || (y+Height < 0) ) return;

		if (bSelected){View.dc->FillSolidRect(0,y,1024,Height,crSelect);}
	...
	void AddDelete(ViewInfo& View,int x,int y)
	{
		...
		if (bSelected)AddIcon(View,View.client->right-16,y,ICON_DELETE,NONE,NONE);
	}
	...
	{
		if (View.bMultiSelected) return;
		if ( (y > View.client->bottom) || (y+16 < 0) ) return;
		if (bSelected) AddIcon(View,0,y,ICON_DROPARROW,0,HS_DROP);
		return;
	}
	int AddComment(ViewInfo& View,int x,int y)
	{
		...
		if ( GetType( ) == nt_int64 )
		{
			...
			if ( gbInt && Val > 140000000 && Val < 80000000000 )
		...
		//if ((GetMemorySize() == 4) && (GetType() != nt_class) )
		...
		if ( GetType( ) == nt_hex64 )
		{
			...
			if (a.GetLength() > 0)
			{
				...
				if (gbString)
				{
					...
					for ( int i=0; i < 4; i++ )
					{
						if (!isprint( (BYTE)txt[i])) bAddStr = false;
					}
...
public:
	...
	virtual void Update(HotSpot& Spot)
	{
		...
		if (Spot.ID == 1)
		//	RequestPosition = ConvertStrToAddress( Spot.Text );
			RequestPosition = atoi(Spot.Text);
	};

	virtual int GetMemorySize()		{int size = 0; for (UINT i=0; i < Nodes.size();i++) size += Nodes[i]->GetMemorySize(); return size;};
	virtual int Draw(ViewInfo& View,int x,int y)
	{
...
public:
	...
	virtual int Draw( ViewInfo& View,int x,int y )
	{
		...
		tx = AddText( View,tx,y,crHex,3, "%0.2X", pMemory[4] ) + FontWidth;
		tx = AddText( View,tx,y,crHex,3, "%0.2X", pMemory[5] ) + FontWidth;
		tx = AddText( View,tx,y,crHex,3, "%0.2X", pMemory[6] ) + FontWidth;
		tx = AddText( View,tx,y,crHex,3, "%0.2X", pMemory[7] ) + FontWidth;
...
public:
	...
	virtual int Draw(ViewInfo& View,int x,int y)
	{
		...
		if (gbText)
		{
			// TODO these are the dots, do alignment instead of 8
...
public:
	...
	virtual int Draw(ViewInfo& View,int x,int y)
	{
		...
		if (Assembly.size()>0) tx = AddOpenClose(View,tx,y);
		tx += FontWidth;
...
public:
	...
	{
		StandardUpdate(Spot);
		if (Spot.ID == 0) memsize = atoi(Spot.Text);
		if (Spot.ID == 1)
		{
...
public:
	CNodeVec2::CNodeVec2()
	{
		for (UINT i=0; i<bOpen.size();i++) bOpen[i] = true;
	}
...
{
public:
	virtual NodeType GetType(){return nt_quat;};
	virtual void Update(HotSpot& Spot)
	{
		StandardUpdate(Spot);
		float v = ( float )atof(Spot.Text);
		...
		if (Spot.ID == 1) WriteMemory(Spot.Address+4,&v,4);
		if (Spot.ID == 2) WriteMemory(Spot.Address+8,&v,4);
		if (Spot.ID == 3) WriteMemory(Spot.Address+12,&v,4);
	};
